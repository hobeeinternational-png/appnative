...
export default function MyHobeeWorkScreen() {
  ...
  const openInboxItem = async (item: MyHobeeWorkInboxItem) => {
    await markRead(item);
    if ((item.item_type as string) === "CASE" && item.reference_id) router.push(`/claims/${item.reference_id}`);
  };
  ...
  return (
    <ScreenContainer containerClassName="bg-[#F6F3ED]" edges={["top", "left", "right"]}>
      <AtmosphericCanvas mood="account">
        <View style={styles.top}><MyHobeeHeader title="WORK CENTER" onBack={() => router.replace("/my-hobee")} notificationCount={snapshot?.customer.unreadWorkItems ?? 0} /><Text style={styles.title}>งานและออเดอร์ของฉัน</Text><Text style={styles.subtitle}>ติดตามสิ่งที่ต้องทำจากทุกบทบาทโดยไม่ต้องสลับหลาย workspace</Text><MyHobeeSegments active="work" /><Pressable onPress={() => router.push("/my-hobee/after-sales")} style={({ pressed }) => [styles.afterSalesLink, pressed && styles.pressed]}><MaterialIcons name="support-agent" size={17} color="#FFFFFF" /><Text style={styles.afterSalesLinkText}>เปิด After-Sales สำหรับผู้ขาย</Text><MaterialIcons name="arrow-forward" size={16} color="#FFFFFF" /></Pressable></View>
