...
function RecentlyViewedSection() {
  ...
  return <Section title="ดูล่าสุด" action="สำรวจเพิ่ม" onAction={() => router.push("/(tabs)/discover")}><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.recentRail}>{items.map((item) => <RecentViewedCard key={item.key} item={item} />)}</ScrollView></Section>;
}

function RecentViewedCard({ item }: { item: RecentlyViewedItem }) {
  const open = () => router.push(item.params ? { pathname: item.route as any, params: item.params } : item.route as Href);
  return <Pressable accessibilityLabel={`เปิดล่าสุด ${item.title}`} onPress={open} style={({ pressed }) => [styles.recentCard, pressed && styles.pressed]}><View><Image source={{ uri: item.image }} style={styles.recentImage} /><View style={styles.recentType}><Text style={styles.recentTypeText}>{item.kind === "product" ? "สินค้า" : item.kind === "travel" ? "ทริป" : "เนื้อหา"}</Text></View></View><Text numberOfLines={2} style={styles.recentTitle}>{item.title}</Text><Text numberOfLines={1} style={styles.recentMeta}>{item.price ?? item.detail}</Text></Pressable>;
}
