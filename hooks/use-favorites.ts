...
export function useFavorites() {
  ...
  useEffect(() => { if (!user) { setIds(new Set()); return; } setLoading(true); void listFavoriteIds(user.id).then(setIds).finally(() => setLoading(false)); }, [user]);
  ...
  return { favoriteIds: ids, loading, toggle, signedIn: Boolean(user) };
}
