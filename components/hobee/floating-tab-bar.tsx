...
const hobeeRouter = router as { push: (href: string) => void; replace: (href: string) => void };
...
export function FloatingBottomNav({ activeKey, onNavigate, notificationCount = 0 }: { activeKey: HobeeNavKey; onNavigate?: (key: HobeeNavKey) => void; notificationCount?: number }) {
  ...
  const navigate = (key: HobeeNavKey) => {
    ...
    if (key === "discover") { router.push("/travel" as never); return; }
  ...
  const handleCenterPress = () => {
    ...
    tapTimer.current = setTimeout(() => { tapTimer.current = null; hobeeRouter.push("/my-hobee"); }, FLOATING_NAV_CONFIG.doubleTapWindowMs);
  };
...
function CenterButton({ onPress, compact = false }: { onPress: () => void; compact?: boolean }) { return <Pressable accessibilityRole="button" accessibilityLabel="เปิด My HOBEE หรือแตะสองครั้งเพื่อสลับ Assistive Mode" onPress={onPress} style={({ pressed }) => [styles.centerButton, { backgroundColor: HOBEE.colors.gold, borderColor: "rgba(255,255,255,0.3)" }, compact && styles.centerButtonCompact, pressed && styles.pressed]}><MaterialIcons name="auto-awesome" size={compact ? 18 : 15} color={HOBEE.colors.darkBase} /><Text style={[styles.centerText, { color: HOBEE.colors.darkBase }, compact && styles.centerTextCompact]}>HOBEE</Text></Pressable>; }
export function FloatingTabBar({ state, navigation }: BottomTabBarProps) { const currentRoute = state.routes[state.index]?.name as HobeeNavKey; return <FloatingBottomNav activeKey={currentRoute} onNavigate={(key) => { if (key === "discover") { router.push("/travel" as never); return; } if (key === "orders") { router.push("/orders"); return; } const route = state.routes.find((item) => item.name === key); if (!route) return; const event = navigation.emit({ type: "tabPress", target: route.key, canPreventDefault: true }); if (!event.defaultPrevented) navigation.navigate(key); }} />; }

const styles = StyleSheet.create({ normalContainer: { position: "absolute", left: HOBEE.nav.outerInset, right: HOBEE.nav.outerInset, zIndex: 20, alignItems: "center" }, barShell: { width: "100%", height: HOBEE.nav.barHeight, position: "relative" }, bar: { width: "100%", height: HOBEE.nav.barHeight, flexDirection: "row", alignItems: "center", justifyContent: "center", borderRadius: HOBEE.nav.barHeight / 2, overflow: "hidden", borderWidth: 1, borderColor: "rgba(255,255,255,0.18)", paddingHorizontal: 12, backgroundColor: "rgba(20,20,18,0.78)", ...HOBEE.elevation.floating }, navGroup: { flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: HOBEE.nav.itemGap }, navCenterGap: { width: HOBEE.nav.centerBreathing }, centerOverlay: { position: "absolute", zIndex: 3, left: "50%", top: -27, marginLeft: -37 }, iconButton: { width: 42, height: 42, alignItems: "center", justifyContent: "center", borderRadius: 21 }, activeButton: { borderWidth: 1, borderColor: "rgba(255,238,185,0.28)", backgroundColor: "rgba(255,194,56,0.18)" }, centerButton: { width: 74, height: 74, alignItems: "center", justifyContent: "center", gap: 0, borderRadius: 37, borderWidth: 4, backgroundColor: "#D6AC48", ...HOBEE.elevation.floating }, centerButtonCompact: { width: FLOAT_SIZE, height: FLOAT_SIZE, marginTop: 0, borderRadius: FLOAT_SIZE / 2, borderWidth: 3 }, centerText: { color: "#1D1A18", fontSize: 11, fontWeight: "900", letterSpacing: -0.5 }, centerTextCompact: { fontSize: 10 }, floating: { position: "absolute", zIndex: 35 }, pressed: { opacity: 0.78, transform: [{ scale: HOBEE.motion.pressScale }] }, badge: { position: "absolute", top: 3, right: 3, minWidth: 14, height: 14, alignItems: "center", justifyContent: "center", borderRadius: 7, backgroundColor: "#DE4A47", paddingHorizontal: 3 }, badgeText: { color: "#FFFFFF", fontSize: 7, fontWeight: "900" } });
